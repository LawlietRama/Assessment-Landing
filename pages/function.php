...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'nanyang.sch.id';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'admission@nanyang.sch.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'Nzhadmission2020';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'admission@nanyang.sch.id';        // Masukkan default from pada email
  $default_email_from_name  = 'Admission Nanyang Zhi Hui School';           // Masukkan default nama dari from pada email
